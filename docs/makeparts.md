...
- `mc.exe` の生成物 (ヘッダー / `.rc` / `.bin`) はコンパイル対象のソースに近い中間ファイルとして `GENDIR` (`gen/`) に置きます。`rc.exe` によるコンパイル結果 (`.res` / `.res.obj`) は他の `.o`/`.obj` と同様に `OBJDIR` (`obj/`) に置きます。`GENDIR` は `OBJDIR` と異なり CRT (ランタイム ライブラリ) ごとのサブディレクトリを持たず、単一の `gen/` を共有します。
- メッセージ コンパイラのフラグは `MCFLAGS` (既定 `-U -cp utf-8`)、リソース コンパイラのフラグは `RCFLAGS`、`.res` から COFF オブジェクトへの変換コマンドは `CVTRES` (既定 `cvtres.exe`) で上書きできます。`-cp utf-8` は生成される `.h`/`.rc` の文字コードをビルド環境のシステム ロケールに依存させず UTF-8 (BOM 付き) に固定します。
- 静的ライブラリのリソース対応は Windows の `.lib` のみです。Linux では `.rc` / `.mc` を無視するため、`.a` には含めません。
- 同名 stem の `.mc` と `.rc` を同一ディレクトリに置かないでください (どちらも `<name>.res` を生成し衝突します)。
- `.mc` / `.rc` はビルドのスキップ判定 (署名) の対象です。これらを変更すると次回ビルドで再コンパイル・再リンクが実行されます。
...
- `.l`/`.y` はビルドのスキップ判定 (署名) の対象です。これらを変更すると次回ビルドで再変換・再コンパイル・再リンクが実行されます。

**GENDIR_EXTRA_C: flex/bison 以外の手段で `gen/` に C ソースを生成する場合**

ヘッダー解析ツールなど、flex/bison 自体ではなく `.l`/`.y` から生成した独自ツールが `gen/` へ C ソースを生成するケースでは、`GENDIR_EXTRA_C` にそのファイルのパスを追加し、対応する Make ルールをアプリ側の `makepart.mk` で定義します。コンパイル (`gen/*.c` → `obj/*.o`/`.obj`) は上記と同じ機構によって処理されます。
...
obj を削除しても bin が再リンクされない場合があります (make が bin を最新と判定するため)。

`TEST_SRCS` からソースを除去した場合は、テスト ディレクトリで次の項目を手動で削除してから `make test` を実行してください。

1. 残存シンボリック リンク (`rm {テストdir}/{除去したソース}`)
2. `obj/{除去したソース}.o` と対応する `.d` / `.gcno`
3. `bin/{テスト名}` (再リンクを強制)

シンボルの状態は `nm bin/{テスト名} | grep {関数名}` で確認できます。T (強シンボル = 実体が残存) なら削除漏れ、W (弱シンボル) ならモックが有効です。  
