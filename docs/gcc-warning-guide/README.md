...
本ガイドは、GCC の警告を品質ゲートとして活用し、EL8、EL9、EL10 の各環境でビルドを破綻させずにロバストな C / C++ プログラミングを行うことを目的としています。
...
- GCC 14 で warning → error に昇格した項目に依存しないコードを書きます
...
- 明示的なキャストはこれらの警告を抑制するため、キャスト直前の範囲検査または理由コメントはコーディング規範の「整数演算の安全性」で担保します  
