...
| 既存カバー | -Wall -Wextra またはデフォルトですでに有効な警告は重複追加しません |
| 対象環境 | EL8、EL9、EL10 のすべてで利用可能であること |

除外理由を以下のカテゴリに分類しています。

| カテゴリ | 説明 |
|---------|------|
| A. すでに有効 | -Wall / -Wextra / デフォルトに含まれるため重複します |
| B. 抑制指定 | =0 による警告抑制であり、基本方針に違反します |
| C. スタイル制約 | コーディング スタイルの強制であり、バグ検出ではありません |
| D. 最適化情報 | コンパイラ最適化に関するフィードバックであり、コードの正しさと無関係です |
| E. 高ノイズ | 正当なコードに対する警告が多く、品質ゲートとして機能しません |
| F. 環境非対応 | 対象環境 (GCC 8 以降) で利用できません |
...
推奨セットに -Wextra を含めているため、-W を併記する必要はありません。  
...
型変換の安全性を包括的に検出するには -Wconversion / -Wsign-conversion を使います。  
これらは推奨セットに含めています。  
明示キャストの正当性 (範囲検査または理由コメント) は、警告では検出できないため  
コーディング規範の「整数演算の安全性」で担保します。
...
文の後の宣言が合法です。対象環境 (GCC 8: gnu11、GCC 14: gnu17) は  
...
コードの正しさや移植性とは無関係です。  
出力の有無はコンパイラ バージョンや最適化レベルに強く依存し、  
GCC 8 と GCC 14 で挙動が異なる可能性が高いです。品質ゲートには適しません。
...
センチネル値との比較など、正当なパターンも多くあります。  
一律に警告すると誤検出が多発し、品質ゲートとして機能しません。

本プロジェクトのコードは整数演算が中心であり、  
浮動小数点演算の安全性はコード レビューで対応する方が実用的です。
...
-Wformat-overflow は GCC 7 以降で -Wall に含まれており (レベル 1)、  
...
|------|------|
| 除外カテゴリ | B. 抑制指定 |
| 概要 | -Wimplicit-fallthrough による switch フォール スルー検出を無効化 |

=0 は警告の抑制指定であり、基本方針に違反します。

-Wimplicit-fallthrough は GCC 7 以降で -Wextra に含まれており、  
推奨セットではこれをそのまま有効にしています。  
意図的なフォール スルーには `__attribute__((fallthrough))` または  
コメント アノテーションで明示してください。
...
inline 展開の可否はコンパイラ バージョンや最適化レベルに依存するため、  
GCC 8 と GCC 14 で異なる結果が出る可能性が高く、  
品質ゲートとして安定しません。
...
|------|------|
| 除外カテゴリ | A. すでに有効 |
| 概要 | 整数からポインターへのキャストでサイズが異なる場合に警告 (C 専用) |
...
ビルド インフラの構成管理に関する警告であり、  
コードの品質やバグの検出とは無関係です。  
プリコンパイル済みヘッダーの管理はビルド システム側で対応すべき問題であり、  
コード品質ゲートの範囲外です。
...
exit() や abort() を呼び出すユーティリティ関数すべてで警告が発生するため、  
ノイズが多くなります。
...
|------|------|
| 除外カテゴリ | A. すでに有効 |
| 概要 | ポインターから整数へのキャストでサイズが異なる場合に警告 (C 専用) |
...
ヘッダー ファイルの防御的インクルード設計 (複数のヘッダーが同じ宣言を  
...
真に危険なエイリアシング違反はコード レビューや静的解析ツールで対応する方が  
実用的です。
...
対象環境の下限が GCC 8 (EL8) であるため、利用できません。
...
対象環境 (GCC 8: gnu11、GCC 14: gnu17) では言語仕様として正当です。  
デバッグ マクロやログ マクロで広く使用されるパターンであり、  
これを禁止する合理的な理由はありません。

この警告は、`-Wdeclaration-after-statement` と同様に、C99 以降の正当な機能を対象とします。  
こうした制約の適用は、モダナイゼーションの方向性と相反します。
