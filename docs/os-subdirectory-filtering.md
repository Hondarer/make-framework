...
本ドキュメントは、makemain.mk におけるサブディレクトリの OS フィルタリング機能について説明します。

## 概要

makemain.mk では、サブディレクトリの再帰的 make 処理に先立ち、ディレクトリ名に基づく OS フィルタリングを実行します。これにより、プラットフォーム固有のコードをディレクトリ単位で分離できます。
...
この場合、`exampleapp/` レベルでは `platform/` が両 OS で有効となり、`platform/` 内で再帰的に make が実行される際に `linux/` または `windows/` がフィルタリングされます。
