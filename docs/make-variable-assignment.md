...
- **遅延展開 (recursively expanded variable)**: `=` を使用します。参照時に毎回右辺を評価し、値を使用するタイミングで展開されます。
- **即時展開 (simply expanded variable)**: `:=` を使用します。代入した時点で 1 回だけ展開して結果の文字列を保存するため、後から参照先が変わっても値は変わりません。
- **追記 (append)**: `+=` を使用します。既存の値に文字列を追加します。
- **条件付き代入 (conditional assignment)**: `?=` を使用します。変数が未定義の場合にのみ値を代入します。新しく定義される場合の種類は遅延展開です。
...
この例では、A は参照時点の B を評価するため two、C は定義時点の B の値を確定して保持するため one になります。

## += (追記) の挙動

"+=" は、変数に文字列を追加します。すでに値がある場合は、先頭に 1 つの空白を挿入してから追加します。未定義の変数に対して最初に使用した場合は、"=" と同じく遅延展開の変数として定義されます。

ただし、すでに変数が定義されている場合は、もとの変数の種類によって挙動が変わります。

- もとが 即時展開 (":=") の場合
    - 追加するテキストを先に展開してから、既存の値に追加します
    - 等価変換のイメージは次の通りです
...
と記述した場合、この時点で $(CFLAGS) が展開され、includes が未定義であればその参照が空文字列に展開されて失われます。その結果、後から includes を定義しても反映されません。
...
- 常に最新の他変数の値を反映したいときは "=" を使用します。
- 定数、コマンド列、重い関数や $(shell …) の結果を何度も使うときは ":=" を使用します。
- 既存の変数に安全に追加したいときは "+=" を使用します。もとの変数の種類 (= か :=) で評価タイミングが変わることに注意します。
...
- 意図せずに "=" を使用すると、$(shell …) や関数展開が参照のたびに実行され、ビルドが遅延することがあります。
- 逆に、":=" で値を固定すると、後から上書きした変数の変化が反映されません。
- "+=" は、未定義なら遅延展開で定義し、定義済みなら元の種類に従って追加します。特に "=" で定義した変数に "+=" した場合、追加部分は未展開のまま後から参照時に評価されます。
- "?=" は未定義かどうかだけを判定します。空文字であっても一度設定されていれば「設定済み」と見なされるため、値は代入されません。即時展開の既定値としたい場合は origin 関数で分岐して ":=" を使用します。
