...
makefw では、`app/makefile` 用の app 直下テンプレート (`__app_template.mk`)、`app/<app_name>/makefile` 用の各 app 直下テンプレート (`__each_app_template.mk`)、配下のビルド層・走査層用の統一テンプレート (`__template.mk`) を使い分けます。  
この方針により保守性は上がりますが、テンプレートを更新した後、すでに配置済みの `makefile` は自動では追従しません。

Linux ではシンボリック リンクで吸収できる場合がありますが、Windows を含むワークスペースでは実体ファイル運用が必要です。  
そのため、テンプレートの実体コピーを一括で再同期するコマンドが必要になります。

## 動作

1. スクリプト自身の配置場所から上方向に `.workspaceRoot` を探し、ワークスペース ルートを特定します。
2. 利用可能なテンプレート一覧を確認します。
3. ワークスペース配下の `makefile` を再帰的に列挙します。
4. 先頭行が既知のテンプレート識別子を持つファイルだけを同期候補として扱います。
5. 識別子と配置場所の組み合わせを検証し、整合する同期元テンプレートを選択します。
6. 内容が同一なら `[スキップ]`、差分があれば `--dry-run` 時は `[対象]`、通常実行時は上書きして `[更新]` を表示します。
7. 識別子に対して配置場所が不正な `makefile` はエラーとして報告し、更新しません。
...
- テンプレート由来 `makefile` に固有設定を直接記述しないでください。差分は次回同期で失われます。
- 配下のビルド層・走査層の固有設定は `makepart.mk` / `makechild.mk` / `makelocal.mk` に移行してください。
- 各 app 直下テンプレートは `app/<app_name>/makefile` に、app 直下テンプレートは `app/makefile` にのみ配置できます。
